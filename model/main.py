...
def main():
    ...
    # for epoch_th in [9]:
        if train_type == 1:
            # file_folder_path = '../DRL_2_model_duanjie_4_3_2/results/net_param/' + str(100)
        ...
        with open("./results/" + str(epoch_th + 1) + train_type_list[train_type] + str(epoch_th + 1) + "_result.txt", "w+") as file:
            ...
            # sys.stdout = file
            print("=========================================" + str(epoch_th + 1) + "=========================================")
            # trained_agent = train(epoch_th + 1, 100, 450, 4, False)
...
#         os.makedirs('./results/net_param')
#     trained_agent = train(100, 450, 4)
...
def train(result_num, epochs, iterations, episodes, Pre_Train, file_folder_path):
    ...
    for epoch in range(epochs):
        ...
        for iter in range(iterations):
            ...
            for ep in range(episodes):
                ...
                for t in count():
                    ...
                    total_reward += reward
                    if done:
                        # if len(agent.buffer) >= agent.batch_size: agent.update(iter)
        ...
        accuracy_epoch = test_epoch(agent.copy())
        accuracy_epochs.append(accuracy_epoch)
    end = time.time()
    savemat(reletive_path + train_type + 'mat/DRL_accuracy_epochs.mat', {'DRL_accuracy_epochs': accuracy_epochs})
    print("Completed %d epoches %d iterations of %d episodes in %.3f s" % (epochs, iterations, episodes, end - start))
    #############plot reward_loss_epochs###################
    plt.close()
    plt.figure()
    plt.plot(reward_loss_epochs)
    plt.xlabel('Number of epochs')
    plt.ylabel('Train value of reward')
    plt.xlim(0, len(reward_loss_epochs))
    plt.ylim(0, max(reward_loss_epochs))
    plt.savefig(reletive_path + train_type + 'picture/DRL_reward_loss_epochs.tif', bbox_inches='tight')

    savemat(reletive_path + train_type + 'mat/DRL_reward_loss_epochs.mat', {'DRL_reward_loss_epochs': reward_loss_epochs})
    # plt.close()

    #############plot cross_entropy_epochs###################
    plt.close()
    plt.figure()
    plt.plot(cross_entropy_epochs)
    plt.xlabel('Number of Epochs')
    plt.ylabel('Train loss of cross entropy')
    plt.ylim(0, np.max(cross_entropy_epochs))
    plt.xlim(0, len(cross_entropy_epochs))
    plt.savefig(reletive_path + train_type + 'picture/DRL_cross_entropy_epochs.tif', bbox_inches='tight')
    # plt.close()
    savemat(reletive_path + train_type + 'mat/DRL_cross_entropy_epochs.mat', {'DRL_cross_entropy_epochs': cross_entropy_epochs})

    final_test_acc = test(reletive_path, agent.copy())
    del agent, accuracy_epochs, cross_entropy_epochs, reward_loss_epochs, env
    gc.collect()
...
def test_epoch(agent):
    ...
    print("=" * 50)
    del env, agent, samples_action, reward_all
    gc.collect()
    return acc
