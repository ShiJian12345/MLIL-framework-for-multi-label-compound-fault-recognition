...
folder = 'processing_data/ten_sample/'
...
def load_mat_func(file_path, file_name, name_end):
    ...
    ten_samples_points = data[100:110].flatten()
    savemat(folder+'HXD-800_11classes_10sample-'+name_end+'.mat', {'800_11classes_10sample-'+name_end: ten_samples_points})
    return ten_samples_points
...
def plot_data_wavelet_graph_wubiankuang(all_sample, name_end):
    ...
    # plt.savefig('data_wavelet_graph_1sample.tif',bbox_inches='tight')
    plt.savefig(folder+'WUBIANKUANG_HDX-800_MSIF-DARL_10sample-'+name_end+'.tif', bbox_inches='tight')
    # plt.savefig('duanjie_data_wavelet_graph_1sample.tif', bbox_inches='tight')
    plt.show()
...
def plot_data_wavelet_graph_you_biankuang(all_sample, name_end):
    ...
    # plt.savefig(folder+'WUBIANKUANG_duanjie_ppo_1sample.tif', bbox_inches='tight')
    plt.savefig(folder+'YOUBIANKUANG_HDX-800_MSIF-DARL_10sample-'+name_end+'.tif', bbox_inches='tight')
    plt.show()
